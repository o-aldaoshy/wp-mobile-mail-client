...
const SnoozeOption: React.FC<{ label: string; time: string; onClick: () => void; }> = ({ label, time, onClick }) => (
...
export const SnoozeModal: React.FC<SnoozeModalProps> = ({ isOpen, onClose }) => {
    if (!isOpen) {
        return null;
    }

    const handleSnooze = (option: string) => {
        alert(`Snoozed: ${option}`);
        onClose();
    };
    ...
    const snoozeOptions = [
        { label: 'Later Today', time: laterTodayTime, handler: () => handleSnooze(`Later Today, ${laterTodayTime}`) },
        { label: 'Tomorrow', time: tomorrowTime, handler: () => handleSnooze(`Tomorrow, ${tomorrowTime}`) },
        { label: 'This Week', time: thisWeekendTime, handler: () => handleSnooze(`This Week, ${thisWeekendTime}`) },
        { label: 'Next Week', time: nextWeekTime, handler: () => handleSnooze(`Next Week, ${nextWeekTime}`) },
        { label: 'Pick Date', time: '', handler: () => handleSnooze('Pick Date') },
    ];
            ...
            >
                <ul>
                    {snoozeOptions.map((option, index) => (
                        <li key={index}>
                            <SnoozeOption label={option.label} time={option.time} onClick={option.handler} />
                        </li>
                    ))}
                </ul>
            </div>
